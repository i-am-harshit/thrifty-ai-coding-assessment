...
exports.signup = async (req, res, next) => {
  ...
  try {
    ...
    // Accept username from client; fallback to email when not provided
    const username =
    ...
    if (err && err.code === 11000) {
      // detect which field caused duplicate (username or email)
      const dupField = Object.keys(err.keyValue || {})[0] || "field";
      err.statusCode = 409;
      err.message = `${dupField} already exists.`;
    }
    next(err);
  }
};
